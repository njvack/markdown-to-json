...
class Option(LeafPattern):
    ...
    def parse(class_, option_description: str) -> Option:
        ...
        options, description = re.split(r"(?:  )|$", option_description.strip(), flags=re.M, maxsplit=1)
    ...
    while left != "":
        ...
        for transform_name, transform in transformations.items():
            ...
            if no_collisions:
                ...
                if similar:
                    if transform_name:
                        print(f"NB: Corrected {short} to {similar[0].short} " f"via {transform_name}")
                    break
...
def parse_options(docstring: str) -> list[Option]:
    ...
    return [Option.parse(start + rest) for (start, rest) in zip(parts[0::2], parts[1::2])]
